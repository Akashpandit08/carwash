                ...
                <div class="card-body">
                    <form action="{{ route('admin.services.update', $service->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" required value="{{ old('name', $service->name) }}">
                            @error('name')
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('service_city_id') is-invalid @enderror" id="service_city_id" name="service_city_id">
                                    ...
                                    @foreach($cities as $city)
                                        <option value="{{ $city->id }}" {{ old('service_city_id', $service->service_city_id) == $city->id ? 'selected' : '' }}>
                                            {{ $city->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('service_zone_id') is-invalid @enderror" id="service_zone_id" name="service_zone_id">
                                    ...
                                    @foreach($zones as $zone)
                                        <option value="{{ $zone->id }}" {{ old('service_zone_id', $service->service_zone_id) == $zone->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('price') is-invalid @enderror" id="price" name="price" step="0.01" required value="{{ old('price', $service->price) }}">
                            @error('price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('duration_minutes') is-invalid @enderror" id="duration_minutes" name="duration_minutes" required value="{{ old('duration_minutes', $service->duration_minutes) }}">
                        ...
                        @if($service->image_url)
                            <div class="mb-3">
                                <img src="{{ $service->image_url }}" alt="{{ $service->name }}" style="max-width: 260px; border-radius: 12px;">
                            </div>
                        @endif
