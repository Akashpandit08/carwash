...
export const AdminPartnerDetailScreen = ({ route, navigation }: any) => {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.text}>{data.business_name || 'No Business Name'}</Text>
        </View>
        <View style={styles.row}>
          <Ionicons name="location-outline" size={20} color="#666" style={styles.icon} />
          <Text style={styles.text}>{data.address || 'No Address'}</Text>
        </View>
        <View style={styles.row}>
          <Ionicons name="cash-outline" size={20} color="#666" style={styles.icon} />
          <Text style={styles.text}>Commission: {data.commission_value || 0} ({data.commission_type || 'percentage'})</Text>
        </View>
      </View>
      ...
      <View style={styles.actions}>
        <TouchableOpacity style={styles.actionBtn} onPress={handleToggle}>
          ...
          <Text style={styles.actionBtnText}>
            {data.current_status === 'active' ? 'Deactivate Partner' : 'Activate Partner'}
          </Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
